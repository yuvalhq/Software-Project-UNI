...
/**
 * Receive a symmetric matrix and run the Jacobi algorithm to return the eigenvectors
 * and eigenvalues of the matrix.
 * The function allocates memory for the JacobiResult instance, so it's the caller's responsibility to free it.
...
/**
 * Receive a symmetric matrix, and return the pivot coordinate for it, which is the off-diagonal
...
/**
 * Receive a symmetric matrix, its found pivot and Jacobi parameters, and run the Jacobi iteration on it.
 */
Matrix jacobi_transform_matrix(Matrix mat, Coordinate *pivot, JacobiParameters *jp, size_t n);

/**
 * Receive a symmetric matrix, its found pivot and Jacobi parameters, and run the Jacobi iteration for
 * finding its eigenvalues.
 * This is an efficient implementation taken from a paper we found at:
...
/**
 * Receive two square matrices, and calculate their off diagonal square difference.
 * Assumes that the two matrices are from the same order.
 */
