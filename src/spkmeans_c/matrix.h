 ...
 * The function allocates memory for the matrix, so it's the caller's responsibility to free it.
 */
...
/**
 * Build a matrix from the given file path, and set its order in the given pointers.
 * The function allocates memory for the matrix, so it's the caller's responsibility to free it.
 */
 ...
 * The function allocates memory for the copied matrix, so it's the caller's responsibility to free it.
 */
Matrix copy_matrix(Matrix mat, size_t n, size_t m);

/**
 * Receive two matrices, and their order, and return a new matrix defined by their subtraction.
...
/**
 * Receive two square matrices, and their order, and return a new matrix defined by their multiplication.
 * The function allocates memory for the new matrix, so it's the caller's responsibility to free it.
 */
Matrix matrix_mul(Matrix left, Matrix right, size_t n);

/**
 * Receive a matrix and its order, and return a pointer to a new transposed matrix.
 * The function allocates memory for the new matrix, so it's the caller's responsibility to free it.
 */
Matrix transpose(Matrix mat, size_t n, size_t m);

/**
 * Receive a squared matrix and its order, and return a pointer to a vector of the diagonal vectors.
 * An example usage of this function is to extract the eigenvalues of a diagonal matrix.
 * The function allocates memory for the new vector, so it's the caller's responsibility to free it.
 */
