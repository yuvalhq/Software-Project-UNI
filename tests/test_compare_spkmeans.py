...
def test_wam(inputs_folder: str, others_answer_folder: str):
    for i in range(1, 11):
...
@pytest.mark.parametrize(param[0], param[1])
def test_ddg(inputs_folder: str, others_answer_folder: str):
    for i in range(1, 11):
        our_mat = read_matrix_from_file(inputs_folder + str(i) + ".txt")
...
@pytest.mark.parametrize(param[0], param[1])
def test_gl(inputs_folder: str, others_answer_folder: str):
    for i in range(1, 11):
        our_mat = read_matrix_from_file(inputs_folder + str(i) + ".txt")
...
@pytest.mark.parametrize(param[0], param[1])
def test_jacobi(inputs_folder: str, others_answer_folder: str):
    for i in range(1, 11):
        ...
        other_eigenvalues, other_eigenvectors = read_vector_and_matrix_from_file(
            others_answer_folder + str(i) + "_j_ans.txt"
        )
